...
*double parse(ll* tokens, int start, int finish, int tokensSize)
{
   ...
   for(int i = start; i <= finish; i++, tokens = tokens.next)
   {
      if(strcmp(tokens[i], "(") == 0)
      {
         int j = findParen(tokens.next, start, finish, i);

         printf("j: %i\n", j);
         tokens.x = parse(tokens.next, j - 1, tokensSize));
...
int tokenize(char* string, ll* tokens)
{
   for(int i = 0; i < strlen(string); i++)
   {
      if(isdigit(string[i]) || (string[i] == '-' && isdigit(string[i + 1])))
      {
         ...
         *num = strtod(string, endpos);

         int howMany = endpos - string[i];

         tokens->add(new ll(num, doublep));

         i += (howMany > 1) ? howMany - 1: 0;
      }
      else if(strncmp(string + i, "pi", 2) == 0)
      {
         tokens->add(new ll("pi", charp));
         i++;
      }
      else if(strncmp(string + i, "sqrt", 4) == 0)
      {
         tokens->add(new ll("sqrt", charp));
         j++;
         i += 3;
      }
      switch(string[i])
      {
         case 'e': tokens->add(new ll("e", charp)); break;
         case '+': tokens->add(new ll("+", charp)); break;
         case '-': tokens->add(new ll("-", charp)); break;
         case '*': tokens->add(new ll("*", charp)); break;
         case '/': tokens->add(new ll("/", charp)); break;
         case '(': tokens->add(new ll("(", charp)); break;
